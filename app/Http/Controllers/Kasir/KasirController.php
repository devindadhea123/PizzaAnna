            ...
            foreach ($request->items as $item) {
                ...
                if ($menu) {
                    if ($menu->stok_menu < $qty) {
                        throw new \Exception('Stok menu "' . $menu->nama_menu . '" tidak mencukupi! Stok tersisa: ' . $menu->stok_menu);
                    }
                    $menu->stok_menu -= $qty;
                    $menu->save();
                }
                ...
                $resep = ResepMenu::where('id_menu', $id_menu)
                ...
                foreach ($resep as $r) {
                    $bahan = BahanBaku::find($r->id_bahan);
                    if ($bahan) {
                        $totalJumlah = $r->jumlah * $qty;
                        if ($bahan->stok < $totalJumlah) {
                            throw new \Exception('Stok bahan "' . $bahan->nama_bahan . '" tidak mencukupi! Stok tersisa: ' . $bahan->stok . ' ' . $bahan->satuan);
                        }
                        $bahan->stok -= $totalJumlah;
                        $bahan->save();
                    }
                }
